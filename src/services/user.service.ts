...
export async function loginUser(email: string, password: string) {
	...
	if (user === undefined) {
		...
		(error as any).data = { password: 'Email hoặc password không đúng' };
		throw error;
	}
...
export async function registerUser(userData: User) {
	...
	const [rows]: any = await pool.query(
		'select u.id,u.status,u.full_name,u.email,u.phone,u.reputation,u.total_credit,r.name as roleName from users u inner join roles r on u.role_id = r.id WHERE u.email = ?',
		[email],
	);
	const user = rows[0];
	const tokens = JWTService.generateTokens({
